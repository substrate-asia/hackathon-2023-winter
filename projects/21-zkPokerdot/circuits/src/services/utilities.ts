...
export function sampleFieldElements(babyjub: BabyJub, numBits: bigint, numElements: bigint): bigint[] {
  const arr: bigint[] = [];
  let num: bigint;
...
export function ecDecompress(xArr: bigint[], deltaArr: bigint[], selector: bigint): bigint[] {
  ...
  assert(
    ...
    "Length mismatch. selectorArr.length: " + String(selectorArr.length) + ", deltaArr.length: " + String(deltaArr.length)
  );
  const q = 21888242871839275222246405745257275088548364400416034343698204186575808495617n;
  const ecArr: bigint[] = [];
...
export function decompressDeck(X0: bigint[], X1: bigint[], Y0_delta: bigint[], Y1_delta: bigint[], s: bigint[]): bigint[] {
...
export function ecX2Delta(babyjub: BabyJub, x: bigint): bigint {
  ...
  let delta = babyjub.F.sqrt(
    babyjub.F.div(babyjub.F.sub(babyjub.F.mul(a, xSquare), one), babyjub.F.sub(babyjub.F.mul(d, xSquare), one))
  );
